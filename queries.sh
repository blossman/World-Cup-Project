...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams LEFT JOIN games AS winners on teams.team_id = winners.winner_id LEFT JOIN games AS opponents on teams.team_id = opponents.opponent_id WHERE (winners.year = 2014 AND winners.round = 'Eighth-Final') OR (opponents.year = 2014 AND opponents.round = 'Eighth-Final') ORDER BY name")"
